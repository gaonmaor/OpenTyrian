...
inline static void blit_enemy(SDL_Surface *surface, unsigned int i,
		signed int x_offset, signed int y_offset, signed int sprite_offset);
...
inline static void blit_enemy(SDL_Surface *surface, unsigned int i,
	...
	const int x = enemy[i].ex + x_offset + tempMapXOfs, y = enemy[i].ey
			+ y_offset;
	const unsigned int index = enemy[i].egr[enemy[i].enemycycle - 1]
			+ sprite_offset;
		...
		if (enemyAvail[i] != 1) {
			...
			if (enemy[i].yaccel && enemy[i].yaccel - 89u > mt_rand() % 11) {
				if (player[0].y > enemy[i].ey) {
					...
					if (enemy[i].eyc >= 0
							|| -enemy[i].eyc < enemy[i].yaccel - 89)
						enemy[i].eyc--;
				}
			}

			if (enemy[i].ex + tempMapXOfs > -29
					&& enemy[i].ex + tempMapXOfs < 300) {
				...
				if (enemy[i].freq[j - 1]) {
					...
					if (--enemy[i].eshotwait[j - 1] == 0 && temp3) {
						...
						if (difficultyLevel > 2) {
							enemy[i].eshotwait[j - 1] = (enemy[i].eshotwait[j
									- 1] / 2) + 1;
							if (difficultyLevel > 7)
								enemy[i].eshotwait[j - 1] =
										(enemy[i].eshotwait[j - 1] / 2) + 1;
						}

						if (galagaMode
								&& (enemy[i].eyc == 0
										|| (mt_rand() % 400) >= galagaShotFreq))
							goto draw_enemy_end;
						...
						case 251:
							; /* Suck-O-Magnet */
							const int attractivity = 4
									- (abs(player[0].x - tempX)
											+ abs(player[0].y - tempY)) / 100;
							player[0].x_velocity +=
									(player[0].x > tempX) ?
						...
						default:
							/*Rot*/
							for (int tempCount = weapons[temp3].multi;
								...
								enemyShot[b].sx = tempX
										+ weapons[temp3].bx[tempPos]
										+ tempMapXOfs;
								enemyShot[b].sy = tempY
										+ weapons[temp3].by[tempPos];
								enemyShot[b].sdmg =
										weapons[temp3].attack[tempPos];
								enemyShot[b].tx = weapons[temp3].tx;
								enemyShot[b].ty = weapons[temp3].ty;
								enemyShot[b].duration =
										weapons[temp3].del[tempPos];
								...
								case 1:
									enemyShot[b].syc =
											weapons[temp3].acceleration;
									enemyShot[b].sxc =
											weapons[temp3].accelerationx;

									enemyShot[b].sxm =
											weapons[temp3].sx[tempPos];
									enemyShot[b].sym =
											weapons[temp3].sy[tempPos];
									break;
								case 3:
									enemyShot[b].sxc =
											-weapons[temp3].acceleration;
								...
								if (weapons[temp3].aim > 0) {
									...
									int relative_x = (target_x + 25) - tempX
											- tempMapXOfs - 4;
									if (relative_x == 0)
										relative_x = 1;
									...
									if (relative_y == 0)
										relative_y = 1;
									const int longest_side = MAX(
											abs(relative_x), abs(relative_y));
									enemyShot[b].sxm = roundf(
											(float) relative_x / longest_side
													* aim);
									enemyShot[b].sym = roundf(
											(float) relative_y / longest_side
													* aim);
								}
							}
							break;
			...
			if (enemy[i].launchfreq) {
				if (--enemy[i].launchwait == 0) {
					...
					b = JE_newEnemy(enemyOffset == 50 ? 75 : enemyOffset - 25,
							tempW, 0);
					...
					if (b > 0) {
						...
						if (e->launchtype > 0 && e->launchfreq == 0) {
							if (e->launchtype > 90) {
								...
								int target_x = (player[0].x + 25) - tempX
										- tempMapXOfs - 4;
								if (target_x == 0)
									target_x = 1;
								...
								if (tempI5 == 0)
									tempI5 = 1;
								const int longest_side = MAX(abs(target_x),
										abs(tempI5));
								e->exc = roundf(
										((float) target_x / longest_side)
												* e->launchtype);
								e->eyc = roundf(
										((float) tempI5 / longest_side)
												* e->launchtype);
							}
						}
						...
						do
							temp = mt_rand() % 8;
						while (temp == 3);
						soundQueue[temp] = randomEnemyLaunchSounds[(mt_rand()
								% 3)];
...
void JE_main(void) {
	...
	start_level_first: if (maorMode) {
		char buff[200] = { 0 };
		// front=25, read=26, special=15, ship=13front=25, read=26, special=15, ship=13
		sprintf(buff, "front=%d, read=%d, special=%d, ship=%d",
				player[0].items.weapon[FRONT_WEAPON].id,
				player[0].items.weapon[REAR_WEAPON].id, player[0].items.special,
				player[0].items.ship);
		//JE_outText(VGAScreen, 30, 70, buff, 4, 0);
		printf(buff);

		//player[0].cash = 9999999;
		///////////////////////////////////////////////////////
		//int i = 13;
		//fade_black(10);
		...
		for (uint i = 0; i < 2/*COUNTOF(player[0].items.sidekick)*/; ++i)
	...
	if (!endLevel) // draw HUD
	{
		...
		} else // not galagaMode
		{
			if (twoPlayerMode) {
				if (--shieldWait == 0) {
					...
					&& player[0].shield < player[0].shield_max
					&& power > shieldT) {
				if (--shieldWait == 0) {
					shieldWait = 15;
		...
		for (uint i = 0; i < 2; ++i) {
			uint item_power =
					player[twoPlayerMode ? i : 0].items.weapon[i].power;

			if (old_weapon_bar[i] != item_power) {
				old_weapon_bar[i] = item_power;

				int x = twoPlayerMode ? 286 : 289, y =
						(i == 0) ?
								(twoPlayerMode ? 6 : 17) :
		...
		if (twoPlayerMode || onePlayerAction) {
			...
			if (temp != lastPower) {
				if (temp > lastPower)
					...
					fill_rectangle_xy(VGAScreenSeg, 269, 113 - 11 - lastPower,
							276, 114 - 11 - temp, 0);
	...
	for (int z = 0; z < MAX_PWEAPON; z++) {
		if (shotAvail[z] != 0) {
			...
			if (!player_shot_move_and_draw(z, &is_special, &tempShotX,
					&tempShotY, &damage, &temp2, &chain, &playerNum, &tempX2,
					&tempY2)) {
				...
				if (enemyAvail[b] == 0) {
					...
					if (collided) {
						if (chain > 0) {
							...
							b = player_shot_create(0, SHOT_MISC, tempShotX,
									tempShotY, mouseX, mouseY, chain,
									playerNum);
							shotAvail[z] = 0;
							goto draw_player_shot_loop_end;
						}
						...
						if (armorleft > damage) {
							if (z != MAX_PWEAPON - 1) {
								if (enemy[b].armorleft != 255) {
									...
									enemy[b].armorleft = 0; // MAOR Cheet super kill.
									JE_setupExplosion(tempShotX, tempShotY, 0,
											0, false, false);
								} else {
									JE_doSP(tempShotX + 6, tempShotY + 6,
											damage / 2 + 3, damage / 4 + 2,
											temp2);
								}
							}
									...
									if (enemyAvail[temp3] != 1) {
										...
										if ((temp3 == b)
											...
											if (enemy[temp3].armorleft
												...
												enemy[temp3].armorleft =
														enemy[temp3].edlevel;

											tempX = enemy[temp3].ex
													+ enemy[temp3].mapoffset;
											tempY = enemy[temp3].ey;

											if (enemyDat[enemy[temp3].enemytype].esize
													!= 1)
												JE_setupExplosion(tempX,
														tempY - 6, 0, 1, false,
														false);
											else
												JE_setupExplosionLarge(
														enemy[temp3].enemyground,
														enemy[temp3].explonum
																/ 2, tempX,
														tempY);
										}
									}
							...
							for (temp2 = 0; temp2 < 100; temp2++) {
								if (enemyAvail[temp2] != 1) {
									...
									if ((temp2 == b) || (temp == 254)
										...
										int enemy_screen_x = enemy[temp2].ex
												+ enemy[temp2].mapoffset;

										if (enemy[temp2].special) {
											assert(
													(unsigned int) enemy[temp2].flagnum-1 < COUNTOF(globalFlags));
										...
										if ((enemy[temp2].enemydie > 0)
												&& !((superArcadeMode != SA_NONE)
														&& (enemyDat[enemy[temp2].enemydie].value
																== 30000))) {
											int temp_b = b;
											tempW = enemy[temp2].enemydie;
											int enemy_offset = temp2
													- (temp2 % 25);
											if (enemyDat[tempW].value > 30000) {
												enemy_offset = 0;
											}
											b = JE_newEnemy(enemy_offset, tempW,
													0);
											if (b != 0) {
										...
										if (enemyDat[enemy[temp2].enemytype].esize
												== 1) {
											JE_setupExplosionLarge(
													enemy[temp2].enemyground,
													enemy[temp2].explonum,
													enemy_screen_x,
													enemy[temp2].ey);
											soundQueue[6] = S_EXPLOSION_9;
										} else {
											JE_setupExplosion(enemy_screen_x,
													enemy[temp2].ey, 0, 1,
													false, false);
				...
				} else  // check if shot collided with player
				{
					for (uint i = 0; i < (twoPlayerMode ? 2 : 1); ++i) {
						if (player[i].is_alive
								&& enemyShot[z].sx
										> player[i].x
												- (signed) player[i].shot_hit_area_x
								&& enemyShot[z].sx
										< player[i].x
												+ (signed) player[i].shot_hit_area_x
								&& enemyShot[z].sy
										> player[i].y
												- (signed) player[i].shot_hit_area_y
								&& enemyShot[z].sy
										< player[i].y
												+ (signed) player[i].shot_hit_area_y) {
							...
							if (player[i].invulnerable_ticks == 0) {
								if ((temp = JE_playerDamage(temp, &player[i]))
										> 0) {
									player[i].x_velocity += (enemyShot[z].sxm
											* temp) / 2;
									player[i].y_velocity += (enemyShot[z].sym
											* temp) / 2;
								}
							}
					...
					if (enemyShotAvail[z] == false) {
						...
						if (enemyShot[z].sgr >= 500)
							blit_sprite2(VGAScreen, enemyShot[z].sx,
									enemyShot[z].sy, shapesW2,
									enemyShot[z].sgr + enemyShot[z].animate
											- 500);
						else
							blit_sprite2(VGAScreen, enemyShot[z].sx,
									enemyShot[z].sy, shapesC1,
									enemyShot[z].sgr + enemyShot[z].animate);
					}
				}
		...
		if (explosions[j].ttl != 0) {
			...
			if (explosions[j].y > 200 - 14) {
				...
				if (explosionTransparent)
					blit_sprite2_blend(VGAScreen, explosions[j].x,
							explosions[j].y, shapes6, explosions[j].sprite + 1);
				else
					blit_sprite2(VGAScreen, explosions[j].x, explosions[j].y,
							shapes6, explosions[j].sprite + 1);
	...
	if ((player[0].is_alive && player[0].armor < 6)
			|| (twoPlayerMode && !galagaMode && player[1].is_alive
					&& player[1].armor < 6)) {
		int armor_amount =
				(player[0].is_alive && player[0].armor < 6) ?
						player[0].armor : player[1].armor;
		...
		if ((player[0].is_alive && player[0].armor < 6
			...
			JE_outText(VGAScreen, 140, 178, "WARNING", 7,
					(warningCol % 16) / 2);
	...
	if (firstGameOver) {
		...
		for (temp2 = 0; temp2 < SFX_CHANNELS; temp2++) {
			if (soundQueue[temp2] != S_NONE) {
				...
				JE_multiSamplePlay(digiFx[temp - 1], fxSize[temp - 1], temp2,
						temp3);
	...
	if (debug) {
		...
		debugHist = debugHist
				+ abs((JE_longint) debugTime - (JE_longint) lastDebugTime);
		debugHistCount++;
	...
	if (displayTime > 0) {
		displayTime--;
		JE_outTextAndDarken(VGAScreen, 90, 10, miscText[59], 15,
				(JE_byte) flash - 8, FONT_SHAPES);
		flash += flashChange;
		if (flash > 4 || flash == 0)
			...
			do {
				...
				if (s[0] == ']') {
					...
					case 'G':
						...
						for (i = 0; i < mapPNum; i++) {
							mapPlanet[i] = atoi(
									strnztcpy(buffer, s + 1 + (i + 1) * 8, 2));
							mapSection[i] = atoi(
									strnztcpy(buffer, s + 4 + (i + 1) * 8, 3));
						}
						break;
					...
					case 'g':
						...
						for (uint i = 0; i < COUNTOF(player[1].items.sidekick);
								++i)
					...
					case 'e': // ENGAGE mode, used for mini-games
						...
						for (uint i = 0; i < COUNTOF(player[0].items.sidekick);
								++i)
					...
					case 'L':
						...
						normalBonusLevelCurrent = (strlen(s) > 27)
								& (s[27] == '$');
					...
					case 'Q':
						...
						if (twoPlayerMode) {
							for (uint i = 0; i < 2; ++i)
								snprintf(levelWarningText[i],
										sizeof(*levelWarningText), "%s %lu",
										miscText[40], player[i].cash);
						...
						for (x = 0; x < temp - 1; x++) {
							do
								read_encrypted_pascal_string(s, sizeof(s),
										ep_f);
							while (s[0] != '#');
						}
						...
						if (jumpBackToEpisode1 && !twoPlayerMode) {
							...
							if (superTyrian) {
								...
								superArcadeMode =
										(initialDifficulty == 8) ? 8 : 1;
							}

							if (superArcadeMode < SA_ENGAGE) {
								if (SANextShip[superArcadeMode] == SA_ENGAGE) {
									sprintf(buffer, "%s %s", miscTextB[4],
											pName[0]);
									JE_dString(VGAScreen,
											JE_fontCenter(buffer, FONT_SHAPES),
											100, buffer, FONT_SHAPES);

									sprintf(buffer, "Or play... %s",
											specialName[7]);
									JE_dString(VGAScreen, 80, 180, buffer,
											SMALL_FONT_SHAPES);
								} else {
									JE_dString(VGAScreen,
											JE_fontCenter(superShips[0],
													FONT_SHAPES), 30,
											superShips[0], FONT_SHAPES);
									JE_dString(VGAScreen,
											JE_fontCenter(
													superShips[SANextShip[superArcadeMode]],
													SMALL_FONT_SHAPES), 100,
											superShips[SANextShip[superArcadeMode]],
											SMALL_FONT_SHAPES);
								}

								if (SANextShip[superArcadeMode] < SA_NORTSHIPZ)
									blit_sprite2x2(VGAScreen, 148, 70, shapes9,
											ships[SAShip[SANextShip[superArcadeMode]
													- 1]].shipgraphic);
								else if (SANextShip[superArcadeMode]
										== SA_NORTSHIPZ)
									trentWin = true;

								sprintf(buffer, "Type %s at Title",
										specialName[SANextShip[superArcadeMode]
												- 1]);
								JE_dString(VGAScreen,
										JE_fontCenter(buffer,
												SMALL_FONT_SHAPES), 160, buffer,
										SMALL_FONT_SHAPES);
								JE_showVGA();
					...
					case 'U':
						if (!constantPlay) {
							...
							tempX = atoi(strnztcpy(buffer, s + 3, 3));
							JE_loadPic(VGAScreen, tempX, false);
							memcpy(pic_buffer, VGAScreen->pixels,
									sizeof(pic_buffer));
							...
							memcpy(VGAScreen->pixels, pic_buffer,
									sizeof(pic_buffer));
						}
						break;
						...
						if (!constantPlay) {
							if (!ESCPressed) {
								...
								do {
									...
									if (s[0] != '#') {
										strcpy(
												levelWarningText[levelWarningLines],
												s);
										levelWarningLines++;
									}
								} while (!(s[0] == '#'));
...
bool JE_titleScreen(JE_boolean animate) {
...
#endif
	{
		do {
			...
			if (redraw) {
				...
				if (animate) {
					...
					draw_font_hv_shadow(VGAScreen, 2, 192, opentyrian_version,
							small_font, left_aligned, 15, 0, false, 1);
					...
					if (moveTyrianLogoUp) {
						for (temp = 61; temp >= 4; temp -= 2) {
							...
							blit_sprite(VGAScreenSeg, 11, temp, PLANET_SHAPES,
									146); // tyrian logo
					...
					for (int i = 0; i < menunum; ++i) {
						...
						draw_font_hv(VGAScreen, x - 1, y - 1, menuText[i],
								normal_font, centered, 15, -10);
						draw_font_hv(VGAScreen, x + 1, y + 1, menuText[i],
								normal_font, centered, 15, -10);
						draw_font_hv(VGAScreen, x + 1, y - 1, menuText[i],
								normal_font, centered, 15, -10);
						draw_font_hv(VGAScreen, x - 1, y + 1, menuText[i],
								normal_font, centered, 15, -10);
						draw_font_hv(VGAScreen, x, y, menuText[i], normal_font,
								centered, 15, -3);
					}
			...
			draw_font_hv(VGAScreen, VGAScreen->w / 2, 104 + menu * 13,
					menuText[menu], normal_font, centered, 15, -1);
			...
			for (unsigned int i = 0; i < SA_ENGAGE; i++) {
				...
				if (arcade_code_i[i] > 0
						&& arcade_code_i[i] == strlen(specialName[i])) {
					if (i + 1 == SA_DESTRUCT) {
						loadDestruct = true;
						...
						JE_clr256(VGAScreen);
						JE_outText(VGAScreen, 10, 10,
								"Cheat codes have been disabled.", 15, 4);
						...
						if (initialDifficulty == 8)
							JE_outText(VGAScreen, 10, 40,
									...
									15, 4);
						JE_outText(VGAScreen, 10, 60, "Prepare to play...", 15,
								4);

						char buf[10 + 1 + 15 + 1];
						snprintf(buf, sizeof(buf), "%s %s", miscTextB[4],
								pName[0]);
						JE_dString(VGAScreen, JE_fontCenter(buf, FONT_SHAPES),
								110, buf, FONT_SHAPES);
						...
						if (select_episode() && select_difficulty()) {
							...
							JE_dString(VGAScreen,
									...
									30, superShips[0], FONT_SHAPES);
							JE_dString(VGAScreen,
									JE_fontCenter(superShips[i + 1],
											SMALL_FONT_SHAPES), 100,
									superShips[i + 1], SMALL_FONT_SHAPES);
							tempW = ships[player[0].items.ship].shipgraphic;
							if (tempW != 1)
								blit_sprite2x2(VGAScreen, 148, 70, shapes9,
										tempW);
							...
							if (superArcadeMode == SA_NORTSHIPZ) {
								for (uint i = 0;
										i < COUNTOF(player[0].items.sidekick);
										++i)
									player[0].items.sidekick[i] = 24; // Companion Ship Quicksilver
							}
						} else {
					...
					case 0: /* New game */
						...
						if (select_gameplay()) {
							...
							if (onePlayerAction) {
								...
								ulong initial_cash[] = { 10000, 15000, 20000,
										30000, 35000 };

								assert(
										episodeNum >= 1 && episodeNum <= EPISODE_AVAILABLE);
		...
		} while (!(quit || gameLoaded || jumpSection || play_demo
				|| loadDestruct));

		trentWinsGame: fade_black(15);
	}
...
void JE_readTextSync(void) {
	...
	JE_dString(VGAScreen, 10, 160, "Waiting for other player.",
			SMALL_FONT_SHAPES);
	JE_showVGA();
...
void JE_createNewEventEnemy(JE_byte enemyTypeOfs, JE_word enemyOffset,
	...
	if (eventRec[eventLoc - 1].eventdat2 != -99) {
		switch (enemyOffset) {
		case 0:
			enemy[b - 1].ex = eventRec[eventLoc - 1].eventdat2
					- (mapX - 1) * 24;
		...
		case 50:
			if (background3x1) {
				...
				enemy[b - 1].ex = eventRec[eventLoc - 1].eventdat2 - mapX3 * 24
						- 24 * 2 + 6;
			}
...
void JE_eventSystem(void) {
	...
	case 5:  // load enemy shape banks
	{
		Uint8 newEnemyShapeTables[] = {
				eventRec[eventLoc - 1].eventdat > 0 ?
						eventRec[eventLoc - 1].eventdat : 0,
				eventRec[eventLoc - 1].eventdat2 > 0 ?
						eventRec[eventLoc - 1].eventdat2 : 0,
	...
	case 16:
		if (eventRec[eventLoc - 1].eventdat > 9) {
			...
			soundQueue[3] = windowTextSamples[eventRec[eventLoc - 1].eventdat
					- 1];
		}
		break;
	...
	case 19: /* Enemy Global Move */
	{
		...
		for (int i = initial_i; i < max_i; i++) {
			if (all_enemies
					|| enemy[i].linknum == eventRec[eventLoc - 1].eventdat4) {
	...
	case 33: /* Enemy From other Enemies */
		if (!((eventRec[eventLoc - 1].eventdat == 512
			...
			if (eventRec[eventLoc - 1].eventdat == 534 && superTyrian)
				eventRec[eventLoc - 1].eventdat = 828
						+ superTyrianSpecials[mt_rand() % 4];
	...
	case 64:
		if (!(eventRec[eventLoc - 1].eventdat == 6 && twoPlayerMode
				&& difficultyLevel > 2)) {
			smoothies[eventRec[eventLoc - 1].eventdat - 1] = eventRec[eventLoc
					- 1].eventdat2;
	...
	case 75:
		...
		if (temp_no_clue) {
			...
			do {
				temp = (mt_rand()
						% (eventRec[eventLoc - 1].eventdat2 + 1
								- eventRec[eventLoc - 1].eventdat))
						+ eventRec[eventLoc - 1].eventdat;
			...
			if (eventRec[eventLoc - 1].eventdat4 > 0) { /*Skip*/
				curLoc = eventRec[eventLoc - 1
						+ eventRec[eventLoc - 1].eventdat4].eventtime - 1;
...
void JE_whoa(void) {
	...
	assert(
			(unsigned )VGAScreen2->h * VGAScreen2->pitch >= screenSize
					&& (unsigned )game_screen->h * game_screen->pitch
							>= screenSize);
	...
	do {
		...
		for (i = screenSize - bottomBorder, j = topBorder / 2; i > 0;
				i--, j++) {
			offset = j + i / 8192 - 4;
		...
		memcpy((Uint8 *) VGAScreenSeg->pixels + topBorder,
				TempScreen1 + topBorder, screenSize - bottomBorder);
...
void draw_boss_bar(void) {
	...
	for (unsigned int b = 0; b < bars; b++) {
		unsigned int x =
				(bars == 2) ?
						((b == 0) ? 125 : 185) : ((levelTimer) ? 250 : 155); // level timer and boss bar would overlap

		JE_barX(x - 25, 7, x + 25, 12, 115);
		JE_barX(x - (boss_bar[b].armor / 10), 7,
				x + (boss_bar[b].armor + 5) / 10, 12, 118 + boss_bar[b].color);
